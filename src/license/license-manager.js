...
const LICENSE_STORAGE_KEY = 'dandelion_license';
...
export async function init() {
  ...
  if (cached) {
    ...
    console.log(`[Dandelion] License: ${cached.status} (cached)`);
    return;
  }

  try {
    const result = await browser.storage.local.get(LICENSE_STORAGE_KEY);
    const jwt = result[LICENSE_STORAGE_KEY];
    ...
    if (!jwt) {
      ...
      console.log('[Dandelion] License: none (free plan)');
      return;
    }
    ...
    if (!payload) {
      ...
      _state.isFreePlan = true;
      await setCache({ status: 'none', payload: null });
      console.log('[Dandelion] License: invalid/expired (free plan)');
      return;
    }
    ...
    if (
      ...
      await setCache({ status: 'none', payload: null });
      console.log(
        `[Dandelion] License: version mismatch (${currentVersion} not in [${payload.version_allowed.join(',')}]) (free plan)`,
      );
      return;
    }
    ...
    console.log(`[Dandelion] License: valid (total_limit=${payload.total_limit})`);
  } catch (error) {
    console.error('[Dandelion] License init error:', error);
    _state.status = 'none';
    _state.payload = { ...FREE_PLAN };
    _state.isFreePlan = true;
  }
...
export function getStatus() {
  return {
    ...
    licenseId: _state.payload?.license_id || null,
  };
}
...
export async function saveLicense(jwtString) {
  if (typeof jwtString !== 'string' || jwtString.trim().length === 0) {
    throw new Error('Invalid JWT string');
  }
  await browser.storage.local.set({ [LICENSE_STORAGE_KEY]: jwtString.trim() });
  await clearCache();
  await init();
}
...
export async function getLicenseJWT() {
  try {
    const result = await browser.storage.local.get(LICENSE_STORAGE_KEY);
