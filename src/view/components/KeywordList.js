...
export function KeywordList({ id, value = '', onChange, placeholder = 'Tambah kata kunci...' }) {
  const parse = useCallback((v) => {
    if (!v) return [];
    return v.split(';').map((s) => s.trimStart()).filter(Boolean);
  }, []);
  ...
  return html`
    <div class="keyword-list-container">
      ...
      <div class="keyword-list" id="${id}-list">
        ${items.length === 0 ? html`<div class="keyword-empty">Belum ada kata kunci.</div>` : items.map((item, idx) => html`
          <div class="keyword-item" draggable="true"
          ...
          >
            ...
            <button class="btn-remove" type="button" onClick=${() => removeItem(idx)}>×</button>
          </div>
        `)}
      </div>
