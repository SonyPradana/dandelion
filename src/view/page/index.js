...
import {
  getTodaySummary, getYesterdaySummary, getRange, getMonthTotal, getWeekTotal,
  getOverallBreakdown, getFullHistory, MONTHLY_TARGET, TARGET_MODE,
} from '../../utils/productivityTracker';
import {
  init, getStatus, getDeviceId, getToken, saveToken, removeToken, getRemainingToday,
  ...
  return html`
    ...
    <div class="pane-body">
      <div class="form-group">
        ...
        <input type="text" id="form-skrining-url" placeholder="Masukkan pola URL form skrining" value=${form.url} onInput=${(e) => update('url', e.target.value)} />
      </div>
      <div class="form-group">
  ...
  return html`
    ...
    <div class="pane-body">
      <div class="form-group">
        ...
        <input type="text" id="not-checked-url" placeholder="Masukkan pola URL (pemeriksaan)" value=${form.url} onInput=${(e) => update('url', e.target.value)} />
      </div>
      <div class="form-group">
        <label>Daftar Master 'Tidak Periksa'</label>
        <${KeywordList} id="not-checked-list" value=${form.list} placeholder="Tambah ID baris (rowfrm...)" onChange=${(v) => update('list', v)} />
      </div>
      <div class="delay-settings-grid">
        <div class="form-group-sm">
          <label for="not-checked-automation-delay">Jeda Awal</label>
          <input type="number" id="not-checked-automation-delay" min="500" max="10000" value=${form.automationDelay} onInput=${(e) => update('automationDelay', e.target.value)} />
        </div>
        <div class="form-group-sm">
          <label for="not-checked-item-delay">Jeda Item</label>
          <input type="number" id="not-checked-item-delay" min="100" max="5000" value=${form.itemDelay} onInput=${(e) => update('itemDelay', e.target.value)} />
        </div>
        <div class="form-group-sm">
          <label for="not-checked-reload-delay">Jeda Reload</label>
          <input type="number" id="not-checked-reload-delay" min="500" max="15000" value=${form.reloadDelay} onInput=${(e) => update('reloadDelay', e.target.value)} />
        </div>
      </div>
      <button class="btn btn-primary" onClick=${save}>Simpan</button>
    </div>
...
function ProduktifitasPage() {
  ...
  const fetchData = () => {
    ...
    (async () => {
      const [today, yesterday, overall, history, periodTotal] = await Promise.all([
        getTodaySummary(), getYesterdaySummary(), getOverallBreakdown(), getFullHistory(),
      ...
      if (!status.isFreePlan && status.payload) {
        ...
        const fmt = (ts) => { const d = new Date(ts * 1000); return `${d.getDate()} ${months[d.getMonth()]} ${d.getFullYear()}`; };
  ...
  if (loading || !pd) {
    return html`
      <div class="pane-header">Produktivitas<button class="pane-header-btn" onClick=${fetchData} title="Refresh">↻</button></div>
        ...
        <div class="prod-col">
          ...
          ${licenseInfo ? html`
            <div style="margin-top:16px;padding-top:12px;border-top:1px solid #e5e7eb">
              ...
              <div class="prod-bar-track"><div class="prod-bar-fill" style="width:${licenseInfo.pct}%"></div></div>
              <div style="font-size:12px;color:#888;margin-top:4px">${licenseInfo.used.toLocaleString()} / ${licenseInfo.limit.toLocaleString()} poin (sejak lisensi)</div>
              <div style="font-size:12px;color:#888;margin-top:8px">Periode Token: ${licenseInfo.from} - ${licenseInfo.to}</div>
            </div>
          ` : html`
            <div style="margin-top:16px;padding-top:12px;border-top:1px solid #e5e7eb">
      ...
      <div class="prod-chart-section">
        ...
        <div class="prod-chart">
          ${rangeData.map((day) => {
            ...
            return html`<div class="prod-chart-bar-wrapper"><div class="prod-chart-bar" style="height:${Math.max(pct, 2)}%"></div><div class="prod-chart-label">${label}</div></div>`;
          })}
        </div>
      </div>
    </div>
...
function QuotaTab() {
  ...
  const copyDeviceId = async () => {
    ...
    try {
      ...
      setTimeout(() => { if (btn) btn.textContent = 'Salin'; }, 1500);
    } catch {
  ...
  const activateToken = async () => {
    ...
    if (!val) { setMsg({ type: 'error', text: 'Masukkan token terlebih dahulu.' }); return; }
    try {
      await saveToken(val);
  ...
  return html`
    ...
    <div class="pane-body" id="license-page-content">
      <div class="license-status ${isFree ? 'free' : 'pro'}">
        ${isFree ? html`<span class="license-badge free">FREE</span>` : html`<span class="license-badge pro">PRO</span>`}
      ...
      <div class="device-id-section">
        ...
        <div class="device-id-row">
          ...
          <button class="device-id-copy" id="device-id-copy-btn" onClick=${copyDeviceId}>Salin</button>
        </div>
        <div class="device-id-hint">Gunakan ID ini untuk mendapatkan token</div>
      </div>
      ${infoHtml}
      <div class="pane-title">Aktifkan Token</div>
      <textarea class="license-jwt-input" id="quota-jwt-input" placeholder="Tempel token (JWT) di sini..." value=${jwtInput} onInput=${(e) => setJwtInput(e.target.value)}></textarea>
      <div class="license-actions">
        <button class="license-btn activate" onClick=${activateToken}>Aktifkan</button>
        <button class="license-btn remove" onClick=${removeTokenAction} disabled=${isFree} style=${isFree ? 'opacity:0.4;cursor:not-allowed' : ''}>Hapus Token</button>
      </div>
...
function PageApp() {
    ...
    const handler = (changes, area) => {
      if (area === 'local' && changes.dandelion_terms) {
        ...
        if (shouldShow && !activePopupRef.current) {
          ...
          popup.promise.then(() => { activePopupRef.current = null; });
  ...
  const sidebarTab = (tab, icon, label) => html`
    <button class="tab-btn${activeTab === tab ? ' active' : ''}" data-tab=${tab} onClick=${() => switchTab(tab)}>
