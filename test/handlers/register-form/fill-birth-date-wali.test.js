...
import { fillTanggalLahirWali } from '../../../src/handlers/register-form/fill-birth-date-wali.js';

const html = readFileSync(resolve('test/__fixtures__/register-form/birth-date-wali.html'), 'utf8');
...
describe('fillTanggalLahirWali', () => {
  ...
  describe('success — direct date panel hit', () => {
    it('should click target date cell when year and month match', async () => {
      ...
      const promise = fillTanggalLahirWali('15-06-2026', container);
    ...
    it('should handle early month (January)', async () => {
      ...
      const promise = fillTanggalLahirWali('15-01-2026', container);
      await vi.advanceTimersByTimeAsync(400);
      expect(await promise).toBe(true);
    });
    ...
    it('should handle late month (December)', async () => {
      ...
      const promise = fillTanggalLahirWali('31-12-2026', container);
      await vi.advanceTimersByTimeAsync(400);
      expect(await promise).toBe(true);
    });

    it('should handle / separator format', async () => {
      const container = document.querySelector('.form-data-individu');
      document.body.appendChild(makeDatePopup(2026, 6, 15));
  ...
  describe('year panel navigation', () => {
    it('should navigate to year panel when year does not match', async () => {
      ...
      fillTanggalLahirWali('15-06-2026', container);
      await vi.advanceTimersByTimeAsync(400);
    ...
    it('should select year from year panel when decade includes target', async () => {
      ...
      document.body.appendChild(
        makeYearPopup(2020, 2029, [2020, 2021, 2022, 2023, 2024, 2025, 2026, 2027, 2028, 2029]),
      );
  ...
  describe('failure cases', () => {
    it('should return false when wrapper not found inside container', async () => {
      document.body.replaceChildren(document.createElement('div'));
      const container = document.createElement('div');
      document.body.appendChild(container);

      expect(await fillTanggalLahirWali('15-06-2026', container)).toBe(false);
    });

    it('should return false when container is null', async () => {
      expect(await fillTanggalLahirWali('15-06-2026', null)).toBe(false);
    });
    ...
    it(
      'should return false when popup has no recognizable panel',
      { timeout: 60_000 },
      async () => {
        const container = document.querySelector('.form-data-individu');
        const popup = document.createElement('div');
    ...
    it('should return false when popup disappears mid-navigation', async () => {
      const container = document.querySelector('.form-data-individu');
      document.body.appendChild(makeDatePopup(2025, 6, 15));
      fillTanggalLahirWali('15-06-2026', container);
      await vi.advanceTimersByTimeAsync(400);
      document.body.querySelectorAll('.mx-datepicker-popup').forEach((el) => el.remove());
      await vi.advanceTimersByTimeAsync(250);
      await vi.advanceTimersByTimeAsync(25_500);
    });
  });
